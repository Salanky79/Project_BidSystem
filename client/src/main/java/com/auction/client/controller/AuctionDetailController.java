...
public class AuctionDetailController {
  ...
  private static final String FOLLOW_GOLD_STYLE =
      "-fx-background-color: transparent; -fx-border-color: #D4AF37; -fx-border-radius: 20;"
          + " -fx-background-radius: 20; -fx-text-fill: #D4AF37; -fx-font-size: 12px; -fx-padding:"
          + " 5 16 5 16; -fx-cursor: hand;";
  private static final String FOLLOW_GRAY_STYLE =
      "-fx-background-color: transparent; -fx-border-color: #777777; -fx-border-radius: 20;"
          + " -fx-background-radius: 20; -fx-text-fill: #888888; -fx-font-size: 12px; -fx-padding:"
          + " 5 16 5 16; -fx-cursor: hand;";
        ...
        lv ->
            new ListCell<>() {
              ...
              protected void updateItem(String item, boolean empty) {
                ...
                String initials =
                    author.length() >= 2
                        ? (author.substring(0, 1) + author.substring(author.length() - 1))
                            .toUpperCase()
                        : author.toUpperCase();
                ...
                Platform.runLater(
                    () -> {
                      if (response != null
                          && response.isSuccess()
                          && response.getData()
                              instanceof com.auction.share.DTO.AuctionDetailDTO detail) {
                        this.currentPrice = detail.getCurrentPrice();
                        this.bidStep = detail.getBidStep();
                        this.totalBids =
                            detail.getBidHistory() != null
                                ? detail.getBidHistory().size()
                                : this.totalBids;

                        this.currentPriceLabel.setText(
                            String.format("%.0f VND", this.currentPrice));
                        this.totalBidsLabel.setText(String.valueOf(this.totalBids));
                        this.minBidLabel.setText(
                            String.format(
                                "Minimum bid: %.0f VND", this.currentPrice + this.bidStep));
              ...
              Platform.runLater(
                  () -> {
                    if (response != null && response.isSuccess()) {
                      ...
                      minBidLabel.setText(
                          String.format("Minimum bid: %.0f VND", currentPrice + bidStep));
                      bidInputField.clear();
                    } else {
                      showBidError(
                          response != null ? response.getMessage() : "Lỗi kết nối máy chủ");
                    }
                  }));
              ...
              Platform.runLater(
                  () -> {
                    if (response != null && response.isSuccess()) {
                      ...
                      showAutoBidError(
                          response != null ? response.getMessage() : "Loi ket noi may chu");
                    }
                  }));
    } catch (ValidationException e) {
